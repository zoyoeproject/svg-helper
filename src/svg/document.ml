...
external createElement : dom -> string -> element = "createElement" [@@bs.send]

external createElementNS :
  dom -> string -> string -> element
  = "createElementNS"
  [@@bs.send]
...
type matrix = {a: int; b: int; c: int; d: int; e: int; f: int}

type transform_group = {animVal: transform array; baseVal: transform array}
...
external appendItem : transform array -> transform -> unit = "appendItem"
  [@@bs.send]

external createTransform :
  transform array -> matrix -> transform
  = "createSVGTransformFromMatrix"
  [@@bs.send]

external transform : element -> transform_group = "" [@@bs.get]

external setMatrix : transform -> matrix -> unit = "setMatrix" [@@bs.send]

external getMatrix : transform -> matrix = "matrix" [@@bs.get]

type mouse_event_info = {clientX: int; clientY: int; offsetX: int; offsetY: int}

external stopPropagation : mouse_event_info -> unit = "" [@@bs.send]

external preventDefault : mouse_event_info -> unit = "" [@@bs.send]
