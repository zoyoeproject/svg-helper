let set_name item name =
  match name with None -> () | Some n -> Document.setAttribute item "id" n
...
let mk_use svgname (x, y) =
  Printf.sprintf "<use href='#%s' x='%d' y='%d' width='30' height='30'/>"
    svgname x y
...
let set_cfg_cursor parent svg =
  ...
  @@ Printf.sprintf
       "url('data:image/svg+xml;utf8,<svg height=\"64\" width=\"64\" \
        class=\"default\" %s xmlns=\"http://www.w3.org/2000/svg\">%s</svg>') \
        24 24, auto"
       prop svg
...
let init_dragdrop context parent item =
  ...
  let dragdrop (px, py) (cx, cy) cont =
    ...
    let _ =
      match context.dragdrop with Some (item, cb) -> cb item | _ -> ()
    in
    if cont then context.dragdrop <- None
  in
  let handle_mouse_down minfo =
    pan_state :=
      Event.pan_handler dragdrop !pan_state
        (Event.Down Document.(minfo.clientX, minfo.clientY))
  in
  let handle_mouse_up minfo =
    pan_state :=
      Event.pan_handler dragdrop !pan_state
        (Event.Up Document.(minfo.clientX, minfo.clientY))
  in
  let handle_mouse_move minfo =
    pan_state :=
      Event.pan_handler dragdrop !pan_state
        (Event.Move Document.(minfo.clientX, minfo.clientY))
  in
